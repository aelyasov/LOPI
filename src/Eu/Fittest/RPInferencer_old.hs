...
inferConAlgRRulesNew lenN flt evs logs = 
    let skpWs    = returnWitnesses flt skip evs logs
        skpRs    = map getRewRulPat skpWs
        skpEvs   = map (symEName . head . getLhsRewRulPat . getRewRulPat) skpWs
        nonSkpEs = evs \\ skpEvs
        nonSkpRs = genRulesFromEventsN lenN $ trace ("nonSkpEs: " ++ (show $ length nonSkpEs)) nonSkpEs
    in  skpWs ++ (filterWitnesses flt $ countPats (trace ("nonSkpRs: " ++ (show $ length nonSkpRs)) nonSkpRs \\  skpRs) logs)
